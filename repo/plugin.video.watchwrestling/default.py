...
def GetLatest():
    ...
    url_content = net.http_GET(BASEURL + 'main', headers=headers).content
    try:
...
def GetMedia(url):
    ...
    if '.html' in url:
        ...
        THEHOST=url.split('//')[1].split('/')[0]
        headers={'Referer':ref,'Host': THEHOST,'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.116 Safari/537.36'}
    ...
    if queued == 'true':
        ...
        if url.startswith("http://pwtalk.net"):
            ...
            headers={'Referer':url,'Host': THEHOST}
